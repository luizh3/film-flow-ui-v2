...
void MovieControl::doStart(const int id, PresenterTypeProgramEnum tpProgram)
{
    setIsLoading(true);

    MultiDetailsRequest request;
    request.setTpProgram(TypeProgram::toString(TypeProgramEnumMapper::toEntity(tpProgram)));
    request.setDsLanguage(UiControlManager::instance().languageManager().dsLocaleBCP47());

    _multiController.findById(id, request).then([&](MovieInformation *movieInformation)
                                                { onFetchEnded(movieInformation); });
}

void MovieControl::doRefresh(const int id, PresenterTypeProgramEnum tpProgram)
{
