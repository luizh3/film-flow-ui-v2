...
SectionConfigModel::SectionConfigModel(TypeProgramEnum tpProgram, QString name, QString key)
    : _tpProgram(tpProgram)
    , _name(std::move(name))
    , _key(std::move(key))
{}

TypeProgramEnum SectionConfigModel::tpProgram() const
{
    return _tpProgram;
}

void SectionConfigModel::setTpProgram(TypeProgramEnum newTpProgram)
{
